...
- Performance fixture used 100 maximum-title tasks plus 5,000 maximum-title sessions for 30 repetitions. Latest development p95: 17.0 ms drawer open and 38.2 ms selection update. Packaged p95: 18.3 ms and 38.8 ms. Both are below the 100 ms reference target.
...
- The first feature-PR CI attempt, run [34592349990](https://github.com/ginolyu3360-code/infinite-lofi/actions/runs/34592349990), exposed 720 × 520 and Mini 420 × 250 overflow on the runner's smaller native display, two scene-transition sampling races, and an incorrect universal application of the local-M2 performance threshold. The follow-up tightens compact layouts, polls computed scene colors, and keeps the documented 100 ms reference gate on local hardware while CI still reports its timings and validates the bounded fixture.
